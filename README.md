...
- 기본 쓰기 감각은 Pretendard를 사용하는 타자기형입니다. 상단 선택기에서 고정된 종이와 움직이는 단일 엘리먼트의 `타자기`, MaruBuri 기반의 책 한 쪽 `문학 서재`, 페이지 경계가 없는 Pretendard 기반 `몰입 캔버스`를 같은 원고에서 즉시 비교할 수 있습니다.
...
- 타자기형은 A4와 플래튼의 가로 위치를 고정하고, 낮은 가로 타격면과 작게 매립된 테마 연동 흑연 엘리먼트가 커서를 대신해 움직입니다. 입력할 때는 엘리먼트가 글자 중심에 잠겨 타격한 뒤 다음 글자 칸으로 전진하고, Enter에서는 캐리어 복귀와 줄바꿈이 함께 일어납니다. 문학 서재는 입력한 글자와 조판 부호가 잉크처럼 정착하며, 몰입 캔버스는 현재 문단을 중심에 두고 세션의 단어·문장·문단을 고정 도크에 조용히 쌓아 보여줍니다.
