...
제목의 단계와 첫 행은 별도 편집 안내 배치로 보존하여 커서가 해당 제목 안에
있을 때만 왼쪽 여백에 Markdown 기호와 제목 종류를 설명합니다. 이 안내는
투영 장식이므로 원문·완성본·인쇄 결과에는 들어가지 않습니다.

첫 장에는 제목과 작성자 정보를 배치하고, 본문 문단 첫 칸과 인용·목록
들여쓰기는 가상 칸으로 만듭니다. 본문과 첫 장 정보는 같은 영문 칸 묶음
판별을 사용하여 소문자와 숫자를 두 자까지 한 칸에 모읍니다. 렌더러는 해당
두 글자를 부모 글자 크기에 상대적인 좌우 반 칸으로 표시하며, 즉시 입력
투영과 AI 이어쓰기 미리보기도 같은 판별을 공유합니다. 닫는 문장 부호는 줄
끝 칸에 함께 둘 수 있으며 여는 문장 부호는
...
조판을 기다리는 동안에도 글자와 커서를 보입니다. 페이지 맞춤에서는 현재
원고지만 DOM에 남기고 실제 뷰포트에서 대칭 여백까지 뺀 배율로 맞춰 내부
스크롤을 만들지 않습니다. 너비 맞춤은 전체 원고를 연속 배치하며 같은
커서·원문 매핑을 사용합니다.
...
페이지 맞춤에서는 현재 A4만 표시하지만 인쇄 미디어에서는 숨긴 페이지를
모두 복원하므로 PDF와 인쇄 결과에는 전체 문서가 포함됩니다.
